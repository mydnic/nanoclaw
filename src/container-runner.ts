...
function buildMounts(
  ...
  // Mount host .netrc for git access to private GitHub repos (read-only)
  const netrcPath = path.join(process.env.HOME || '/root', '.netrc');
  if (fs.existsSync(netrcPath)) {
    mounts.push({ hostPath: netrcPath, containerPath: '/home/node/.netrc', readonly: true });
  }
...
/**
 * Write RTK hook to .claude-shared/settings.json if not already present.
 * Called at every spawn; no-op once the file exists.
 */
function ensureRtkSettings(claudeDir: string): void {
